...
from models.request import CreateRetrievalRequest, CreateSubmissionRequest
...
def _validate_destination_path(destination_path: str) -> Path:
...
async def run_archive_retrieval(retrieval_id: int) -> None:
    """Dummy background task placeholder for archive retrieval.

    TODO: implement the full retrieval workflow:
      1. Restore S3 objects from archive tier if needed (poll until thawed).
      2. Download chunked archive parts from ActiveScale into a temp location.
      3. Validate per-part checksums from the manifest.
      4. Reassemble and extract the archive into destination_path.
      5. Validate the extracted BagIt structure.
      6. Mark the retrieval record as COMPLETED (or FAILED on error).
    """
    # Yield to the event loop so uvicorn can flush the HTTP response first.
    await asyncio.sleep(0)
    ...
    with Session(engine) as session:
        ...
        # Placeholder: no real work yet.
        _log_event(
            logging.WARNING,
            "retrieval.run.not_implemented",
            retrieval_id=retrieval_id,
            drive_name=retrieval.drive_name,
        )
...
@app.post(
    ENDPOINT_PREFIX + "/submission/{drive_name}/retrieve",
    status_code=status.HTTP_201_CREATED,
...
async def create_retrieval(
    ...
    try:
        _validate_destination_path(request.destination_path)
