"""Fetch project information from project database and send it to offboarding backend.
Usage: initiate_offboard.py <project_id>
Adapted from a script by Libby Li.
"""
...
class ProjectInfoHandler:
    ...
    def fetch_member_data(self, pid: int) -> list:
        ...
        members = self.api_client.get_project_members(pid, expand=['person','role', 'division','person.identities', 'person.status'])
        columns = ['id','person.email','person.full_name','person.identities','person.status', 'role', 'notes']
        return [
...
if __name__ == "__main__":
    ...
    print(data)
    print("Sending to driveoff backend")
    # Define headers and parameters if POST is needed
    api_url = "https://driveoff-demo.cer-sandbox.cloud.edu.au/api/v1/resdriveinfo"
    headers = {'Content-Type': 'application/json',
            'x-api-key': '0920ccde-b645-4daf-8b88-9bf2154d67fb'}
        ...
        for drive in data['services']['research_drive']:
            ...
            if 'name' in drive:
                ...
                if response:
                     ...
                     print(f"Visit https://uoa-eresearch.github.io/driveoff/?drive={drive['name']}")
                else:
